...
def build_layers(params, neuron_parameters={}):
    ...
    for layer_name in sorted(params.keys()):
        ...
        match layer_type:
            case "LinearTorch":
                last_shape = get_linear_layer_shape(weight_list_from_str(layer_params["weight"]))
            ...
            case "LIFTorch":
                ...
                shape = get_lif_layer_shape(last_shape, weight_list_from_str(layer_params["w_rec"]) if has_rec else None)
...
def drop_exceeding_synapses(weights, max_synapses = 63):
    ...
    for neuron_id,neuron_weights in list(enumerate(weights)):
        ...
        if synapses_count > max_synapses:
            ...
            weights[neuron_id] = np.where(mask, np.zeros(len(neuron_weights)),neuron_weights).astype(float)
            dropped_neurons_count += 1
            dropped_synapses_count += count_to_remove
    return dropped_neurons_count,dropped_synapses_count

def drop_exceeding_layer_output_synapses(lin_layer, prev_lif_layer=None, max_synapses=63):
    ...
    if neurons_count > 0:
        logger.info(f"Dropped {int(synapses_count/neurons_count)} output synapses each for {neurons_count} neurons in layer {lin_layer} (Rec:{prev_lif_layer if prev_had_rec else None})")
